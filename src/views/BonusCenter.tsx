...
export default function BonusCenter({
  ...
  const handleClaimLossRebate = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        showToast(
          `অভিনন্দন! আপনার ৳${res.data.amount} লস রিবেট সফলভাবে ব্যালেন্সে যুক্ত হয়েছে।`,
          "success",
        );
  ...
  const tabs = [
    ...
    { id: "lucky_spin", label: "লাকি স্পিন" },
  ];
  ...
  const renderPWAReminder = () => {
    ...
    return (
      <div className="bg-yellow-400/10 border border-yellow-500/30 rounded-2xl p-5 mb-5 flex items-center justify-between gap-4">
        <div>
          <p className="text-yellow-400 font-black text-xs uppercase tracking-wider mb-1">
            বোনাস পেতে অ্যাপ ইনস্টল করুন
          </p>
          <p className="text-[10px] text-yellow-500/70 font-bold">
            আমাদের অ্যাপটি ইনস্টল করলে আপনি নিরবচ্ছিন্ন অভিজ্ঞতা এবং এক্সকুসিভ
            বোনাস পাবেন!
          </p>
        </div>
        <InstallAppButton />
  ...
  const triggerCoinsExplosion = (count: number) => {
    const burstParticles: CoinParticle[] = Array.from({ length: count }).map(
      (_, i) => {
        ...
        const iconTypes: Array<"coin" | "sparkle" | "goldStar"> = [
          "coin",
          "coin",
          "coin",
          "sparkle",
  ...
  const handleClaimReward = async (amount: number, bonusId: string) => {
    ...
    if (
      userData?.bonusesClaimed?.includes(bonusId) ||
      genericClaiming.includes(bonusId)
    ) {
    ...
    if (
      bonusId !== "registration_bonus" &&
      (!userData?.totalDeposits || userData.totalDeposits <= 0)
    ) {
      showToast(
        "কোন প্রকার বোনাস বা উপহার দাবি করার আগে প্রথম ডিপোজিট করা আবশ্যক",
        "warning",
      );
      onTabChange("deposit");
      return;
    ...
    if (bonusId === "registration_bonus") {
      ...
      if (!isPWA) {
        showToast(
          "এই বোনাসটি শুধুমাত্র আমাদের অ্যাপ থেকে দাবিযোগ্য",
          "warning",
        );
        return;
      }
      ...
      if (!hasDeposit && !hasBankCard) {
        showToast(
          "শর্ত অপূর্ণ: প্রথম ডিপোজিট অথবা উইথড্র ব্যাংক কার্ড যুক্ত থাকতে হবে",
          "warning",
        );
        return;
      }
      ...
      if (dailyStreak < required) {
        showToast(
          `এই বক্সটি খুলতে আপনার অন্তত ${required} দিনের স্ট্রিক প্রয়োজন`,
          "warning",
        );
        return;
      }
  ...
  const nextVip =
    currentVipIndex < vipLevels.length - 1
      ? vipLevels[currentVipIndex + 1]
      : null;

  const vipProgressPercent = nextVip
    ? Math.min(
        Math.max(
          ((totalDeposited - currentVip.target) /
            (nextVip.target - currentVip.target)) *
            100,
          0,
        ),
        100,
  ...
  const handleClaimVipReward = async (
    ...
    if (!userData?.totalDeposits || userData.totalDeposits <= 0) {
      showToast(
        "ভিআইপি বোনাস দাবি করার আগে প্রথম ডিপোজিট করা আবশ্যক",
        "warning",
      );
      onTabChange("deposit");
      return;
    ...
    if (totalDeposited < vipLevels[levelNum - 1].target) {
      showToast(
        "এই ভিআইপি লেভেলে পৌঁছাতে আমানত বা খেলোয়াড় লক্ষ্যমাত্রা অপূর্ণ আছে",
        "warning",
      );
      return;
    }
    ...
    try {
      ...
      showToast(
        `অভিনন্দন! VIP Level ${levelNum} উপহার দাবি করেছেন এবং ৳${rewardAmount} জমা হয়েছে!`,
        "success",
      );
    } catch (err) {
  ...
  const handleClaimCashback = async () => {
    ...
    try {
      ...
      showToast(
        `আপনার ক্যাশব্যাক রিবেট ৳${simulatedCashback} সফলভাবে ব্যালেন্সে যুক্ত হয়েছে!`,
        "success",
      );
    } catch {
          ...
          <div className="space-y-4">
            ...
            <div className="bg-[#0f1926] border border-white/5 rounded-2xl overflow-hidden shadow-xl group">
              <div className="bg-[#1a2533] px-4 py-3 flex items-center justify-between border-b border-white/5">
                ...
                <span className="text-[9px] bg-yellow-400/10 text-yellow-500 px-2.5 py-1 rounded-full font-black uppercase tracking-widest border border-yellow-500/20">
                  APP ONLY
                </span>
              </div>
              <div className="p-5 flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <span className="font-black text-teal-400 text-2xl italic tracking-tighter">
                    ৳{welcomeBonus}
                  </span>
                </div>
                <motion.button
                  whileTap={{ scale: 0.95 }}
                  onClick={() =>
                    handleClaimReward(welcomeBonus || 17, "registration_bonus")
                  }
                  disabled={userData?.bonusesClaimed?.includes(
                    "registration_bonus",
                  )}
              ...
              <div className="flex flex-col items-center text-center space-y-4 relative z-10">
                ...
                {isLoadingLossStats ? (
                  <div className="w-full bg-white/5 rounded-3xl p-6">
                    <LoadingSpinner
                      size="sm"
                      message="Calculating your losses..."
                    />
                  </div>
                ) : (
                  <>
                    <div className="w-full grid grid-cols-2 gap-3">
                      <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                        ...
                        <span className="text-white text-lg font-black italic tabular-nums">
                          ৳{(lossStats?.totalBet || 0).toLocaleString()}
                        </span>
                      </div>
                      <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                        <p className="text-gray-500 text-[9px] font-black uppercase tracking-widest mb-1">
                        ...
                        <span className="text-red-400 text-lg font-black italic tabular-nums">
                          ৳{(lossStats?.netLoss || 0).toLocaleString()}
                        </span>
                      </div>
                    </div>

                    <div className="w-full bg-gradient-to-br from-[#1a2533] to-[#0f1926] rounded-3xl p-6 border border-teal-500/20 shadow-xl">
                      <p className="text-teal-500/70 text-[10px] font-black uppercase tracking-widest mb-1.5">
                        Claimable Bonus (1%)
                      </p>
                      <div className="flex items-baseline justify-center gap-1">
                        <span className="text-white text-5xl font-black italic tabular-nums">
                          ৳{(lossStats?.rebateAmount || 0).toLocaleString()}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 text-[9px] text-gray-500 font-bold uppercase tracking-widest bg-white/5 px-4 py-2 rounded-full border border-white/5">
                      ...
                      <span>
                        ...
                        {lossStats?.lastClaimedAt
                          ? new Date(
                              new Date(lossStats.lastClaimedAt).getTime() +
                                7 * 24 * 60 * 60 * 1000,
                            ).toLocaleDateString()
                          : "Now"}
                      </span>
                ...
                <div className="bg-yellow-400/5 rounded-2xl p-4 border border-yellow-500/10 w-full">
                  <p className="text-[9px] text-yellow-500/70 font-bold uppercase tracking-widest leading-relaxed text-center">
                    গত ৭ দিনের নিট লসের ওপর ১% ক্যাশব্যাক রিবেট পান। এটি সপ্তাহে
                    একবার দাবি করা যায়।
                  </p>
                </div>
              </div>
        ...
        <div className="bg-[#0f1926] rounded-2xl p-5 text-gray-400 text-[11px] leading-relaxed mt-2 border border-white/5 shadow-2xl relative overflow-hidden">
          ...
          <ul className="space-y-2 text-white/60">
            <li className="flex gap-2">
              ...
              <span>
                <b>নিবন্ধন বোনাস:</b> নতুন আইডি তৈরি করার পর যে কেউ আমাদের অ্যাপ
                ব্যবহার করে ১৭ টাকা দাবি করতে পারবেন।
              </span>
            </li>
          </ul>
        </div>
            ...
            <div className="bg-[#0f1926] rounded-[32px] p-6 border border-white/5 shadow-2xl relative overflow-hidden group">
              ...
              <div className="flex items-center justify-between mb-6 relative z-10">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-white font-black text-lg uppercase leading-none block">
                      VIP {currentVip.name}
                    </span>
                    <span className="text-[10px] text-yellow-500 font-black uppercase tracking-[0.2em]">
                      Member Since{" "}
                      {new Date(
                ...
                {nextVip && (
                  <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest">
                    LVL {userData?.vipLevel || 1} / {vipLevels.length}
                  </span>
                )}
              </div>
              ...
              {nextVip ? (
                <div className="space-y-3 relative z-10">
                  ...
                  <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest text-center mt-2">
                    Deposit ৳
                    {(nextVip.target - totalDeposited).toLocaleString()} more to
                    reach <b>{nextVip.name}</b>
                  </p>
                </div>
              ) : (
                <div className="py-2 flex items-center justify-center gap-2 text-teal-400 bg-teal-500/10 rounded-2xl border border-teal-500/20">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-[0.2em]">
                    You have reached Maximum VIP Level!
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="space-y-4">
              {vipLevels.map((lvl, idx) => {
                ...
                return (
                  <div
                    key={lvl.name}
                    className={`bg-[#0f1926] rounded-3xl border transition-all ${isCurrent ? "border-teal-500/30" : isLocked ? "border-white/5 opacity-80" : "border-white/10"}`}
                  >
                    <div className="p-5 flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h5
                            className={`font-black uppercase tracking-tight text-base ${isLocked ? "text-gray-500" : "text-white"}`}
                          >
                            {lvl.name}
                          </h5>
                          <span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest leading-none">
                            Target: ৳{lvl.target.toLocaleString()}
                          </span>
                        </div>
                      </div>
                      <div className="text-right">
                        <span className="text-[9px] text-gray-500 font-black uppercase block tracking-widest mb-1">
                          Bonus Reward
                        </span>
                        <span
                          className={`${isLocked ? "text-gray-700" : "text-teal-400"} font-black text-xl italic tabular-nums leading-none`}
                        >
                    ...
                    <div className="px-5 pb-5 pt-1">
                      <motion.button
                        ...
                        onClick={() =>
                          handleClaimVipReward(levelNum, lvl.reward)
                        }
                        ...
                        className={`w-full py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all border ${
                          isClaimed
                            ...
                            : isLocked
                              ...
                              : claimingVip === bonusId
                                ? "bg-teal-700 text-black border-teal-600"
            ...
            <div className="bg-[#0f1926] rounded-[32px] overflow-hidden border border-white/5 shadow-2xl relative group">
              <div className="p-8 space-y-6 relative z-10">
                ...
                <div className="text-center space-y-2">
                  ...
                  <p className="text-gray-400 text-xs font-bold leading-relaxed max-w-[240px] mx-auto">
                    সুপার ভিআইপি মেম্বার হিসেবে যুক্ত হয়ে পান প্রতিদিন আনলিমিটেড
                    বোনাস এবং ভিআইপি সাপোর্ট।
                  </p>
                </div>
                <div className="space-y-3 py-4">
                  {[
                    ...
                    <div
                      key={feature}
                      className="flex items-center gap-3 bg-white/5 p-3.5 rounded-2xl border border-white/5"
                    >
                      <div className="w-5 h-5 rounded bg-teal-500/10 flex items-center justify-center text-teal-400">
                        <CheckCircle2 size={12} />
                      </div>
                      <span className="text-[11px] font-black uppercase text-gray-300 tracking-wide">
                        {feature}
                      </span>
                    </div>
                  ))}
                ...
                <motion.button
                  whileTap={{ scale: 0.95 }}
                  onClick={() =>
                    window.open(APP_CONFIG.SUPPORT_TELEGRAM, "_blank")
                  }
          ...
          <div className="space-y-4">
            <div className="bg-[#0f1926] rounded-[32px] p-8 border border-white/5 shadow-2xl text-center space-y-6 relative overflow-hidden">
              ...
              <p className="text-[9px] text-gray-600 font-bold uppercase leading-relaxed tracking-wider">
                আমাদের টেলিগ্রাম চ্যানেলে জয়েন করুন নতুন নতুন প্রোমো কোড পেতে।
              </p>
            </div>
          </div>
        )}
        ...
        {activeTab === "lucky_spin" && (
          <div className="space-y-4">
            <LuckySpinWheel
              userData={userData}
              balance={balance}
              onUpdateUser={onUpdateUser || (async () => {})}
              ...
              <div className="flex flex-col items-center text-center space-y-4 relative z-10">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-teal-400 text-[10px] font-black uppercase tracking-[0.3em]">
                    Ready to claim: 1.5% Rate
                  </p>
                </div>

                <div className="w-full bg-white/5 rounded-3xl p-6 border border-white/5 backdrop-blur-sm my-4">
                  <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest mb-1.5">
                    Unclaimed Reward
                  </p>
                  <span className="text-white text-5xl font-black italic tabular-nums">
                    ৳{simulatedCashback.toLocaleString()}
                  </span>
                </div>
                ...
                <motion.button
                  ...
                  onClick={() =>
                    handleClaimReward(
                      simulatedCashback,
                      `cashback_${new Date().toISOString().split("T")[0]}`,
                    )
                  }
                  disabled={userData?.bonusesClaimed?.includes(
                    `cashback_${new Date().toISOString().split("T")[0]}`,
                  ...
                  className={`w-full py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all shadow-2xl ${
                    userData?.bonusesClaimed?.includes(
                      `cashback_${new Date().toISOString().split("T")[0]}`,
                    )
                ...
                <p className="text-[9px] text-gray-500 font-bold uppercase tracking-widest leading-relaxed">
                  আপনার মোট টার্নওভারের ১.৫% প্রতিদিন রাত ১২টায় ক্যাশব্যাক
                  হিসেবে জমা হয়।
                </p>
              </div>
            </div>
